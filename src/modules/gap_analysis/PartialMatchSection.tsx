  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
      <div className="bg-blue-50 px-6 py-4 border-b border-blue-100">
        ...
        <p className="text-sm font-medium text-blue-700 mt-1">Close to job requirements — a small push matters.</p>
      </div>
      ...
      <div className="p-6 grid gap-6 text-left">
        {partialMatches.map((match, i) => {
          ...
          return (
            <div key={`${match.skill_name}-${i}`} className="text-left">
               <div className="flex justify-between items-end mb-2">
                 <h4 className="font-bold text-gray-900">{match.skill_name}</h4>
                 <div className="text-sm font-medium text-blue-600 bg-blue-50 px-2 rounded">
               ...
               <div className="relative pt-1 pb-4">
                 <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden flex">
                    ...
                    <div 
                      className="h-full bg-amber-300 relative"
                 ...
                 {/* Target Marker */}
                 <div 
                    className="absolute top-0 w-1 h-4 bg-gray-800 rounded-sm transform -translate-x-1/2" 
                    style={{ left: `${targetPct}%`, marginTop: '0.125rem' }} 
                    title="Required Target"
                 />
                 
                 <div className="flex justify-between mt-2 text-xs font-medium uppercase tracking-wide">
                   <span className="text-gray-500">You: {match.current_label}</span>
                   <span className="text-gray-800 font-bold ml-auto" style={{ paddingRight: `${100 - targetPct}%`, marginRight: '-1rem' }}>
                      Target
                   </span>
                 </div>
               </div>
